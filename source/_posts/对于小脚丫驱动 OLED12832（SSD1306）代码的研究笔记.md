---
layout: post
title: 对于小脚丫驱动 OLED12832（SSD1306）代码的研究笔记
categories: [FPGA]
date: 2021-12-07 23:17:54
keywords: FPGA, SSD1306
---

# 对于小脚丫驱动 OLED12832（SSD1306）代码的研究笔记。

* 内容为个人理解，不一定正确。

## 1.  数据手册
   [SSD1306数据手册/SSD1306规格书与控制芯片手册.zip · SwetyCore/个人网盘01 - Gitee.com](https://gitee.com/swetycore/personal-network-disk-01/blob/master/SSD1306数据手册/SSD1306规格书与控制芯片手册.zip)
...
## 2. 分析
   ...
   | 引脚     | 作用                                                         |
   | -------- | ------------------------------------------------------------ |
   | oled_dcn | 这是数据/命令控制引脚。当它被拉高(即连接到VDD)，数据D[7:0]作为数据处理。当它被拉低时，D[7:0]处的数据将被传输到命令寄存器。在I2C模式下，该引脚作为SA0来选择从地址。当选择三线串行接口时，此引脚必须连接到VSS。具体与单片机接口信号的关系请参考定时特性。 |
   | oled_dat |                                                              |
   | oled_csn | 这个引脚是芯片选择输入。(低电平有效)                         |
   ...
   | 变量名                                            | 作用                                         |
   | ------------------------------------------------- | -------------------------------------------- |
   | INIT_DEPTH                                        | 初始化的命令的数量                           |
   | IDLE，MAIN，INIT，SCAN，WRITE ， DELAY            | 模块工作状态（值）                           |
   | HIGH/LOW，DATA/CMD                                | 高/低电平，数据/指令选择                     |
   | [7:0] cmd [24:0]                                  | 存储初始化屏幕指令                           |
   | [39:0] mem [122:0];                               | 存储 5*8 字库                                |
   | [7:0]	y_p，x_ph，x_pl;                         | 定位列页地址，定位行地址低位，定位行地址高位 |
   | [(8*21-1):0] char;                                | 存储要显示的字                               |
   | [7:0]	num, char_reg;                           | 要显示的字数量，？将要发送到oled的数据       |
   | [4:0]	cnt_main, cnt_init, cnt_scan, cnt_write; | 各种模块工作状态的子状态（名)                |
   | [15:0]	num_delay, cnt_delay, cnt;              | delay计数，delay设定值，指令发送计数         |
   | [5:0] 	state, state_back;                      | 模块工作状态（名），执行完后要回到的状态     |
   |                                                   |                                              |
   ...
   * #### 屏幕初始化( init 状态)
   ...
   ![image-20211206204542354](https://gitee.com/swetycore/blog-gallery/raw/master/img/fpga/ssd1306INIT.png)
...
* #### Scan
  ...
  SCAN:begin	//刷屏状态，从RAM中读取数据刷屏
  	...
  	case(cnt_scan)
  		...
  		5'd 4:	begin oled_dcn <= DATA; char_reg <= 8'h00; state <= WRITE; state_back <= SCAN; end	//将5*8点阵编程8*8
  		5'd 5:	begin oled_dcn <= DATA; char_reg <= 8'h00; state <= WRITE; state_back <= SCAN; end	//将5*8点阵编程8*8
  		5'd 6:	begin oled_dcn <= DATA; char_reg <= 8'h00; state <= WRITE; state_back <= SCAN; end	//将5*8点阵编程8*8(占三位？？？)
  		5'd 7:	begin oled_dcn <= DATA; char_reg <= mem[char[(num*8)+:8]][39:32]; state <= WRITE; state_back <= SCAN; end
  		5'd 8:	begin oled_dcn <= DATA; char_reg <= mem[char[(num*8)+:8]][31:24]; state <= WRITE; state_back <= SCAN; end
  		5'd 9:	begin oled_dcn <= DATA; char_reg <= mem[char[(num*8)+:8]][23:16]; state <= WRITE; state_back <= SCAN; end
  		5'd10:	begin oled_dcn <= DATA; char_reg <= mem[char[(num*8)+:8]][15: 8]; state <= WRITE; state_back <= SCAN; end
			...
			case(state)
				...
				MAIN:begin
						...
						case(cnt_main)	//MAIN状态
							...
							5'd2:	begin y_p <= 8'hb1; x_ph <= 8'h10; x_pl <= 8'h00; num <= 5'd16; char <= " voltage:  . V  ";state <= SCAN; end
							...
							5'd5:	begin y_p <= 8'hb1; x_ph <= 8'h15; x_pl <= 8'h00; num <= 5'd 1; char <=data[7:4]; state <= SCAN; end
							5'd6:	begin y_p <= 8'hb1; x_ph <= 8'h16; x_pl <= 8'h00; num <= 5'd 1; char <=data[3:0]; state <= SCAN; end
